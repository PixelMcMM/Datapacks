...
worldborder set 500
#reset scoreboard
execute unless entity @s[tag=hhEndInit] as @a[tag=hhHunted,nbt={Dimension:"minecraft:the_end"}] at @s run function hh:init/end_boss_init
...
execute if entity @s[tag=hhWardenInfect] align xyz run summon marker ~ ~-1 ~ {Tags:["hhCatalystMarker","hhMarker"],data:{cursors:[{charge:30,pos:[I;0,0,0],update_delay:0}]}}
...
execute as @e[type=end_crystal,tag=hhECrystal] at @s if score @s hhECTimer >= HealDelay hhECTimer run summon marker ~ ~4 ~ {Tags:["hhECMarker","hhMarker"]}
